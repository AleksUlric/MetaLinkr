...
:: 检查是否有参数
if "%1"=="restart" (
    echo 检测到重启参数，正在重启Admin后端服务...
    goto :restart_service
) else if "%1"=="stop" (
    echo 检测到停止参数，正在停止Admin后端服务...
    goto :stop_service
) else if "%1"=="" (
    echo 正在启动Admin后端服务...
    goto :start_service
    ...
    echo 未知参数: %1
    echo 用法: start-admin-module.bat [start^|stop^|restart]
    pause
    exit /b 1
)
...
:start_service
cd /d "%~dp0..\linkr-server\admin-module"
echo 当前目录: %CD%
...
start "Admin后端服务" cmd /k "mvn spring-boot:run"
...
echo.
echo 使用方法:
echo   start-admin-module.bat        - 启动Admin后端服务
echo   start-admin-module.bat stop   - 停止Admin后端服务
echo   start-admin-module.bat restart - 重启Admin后端服务
echo.
