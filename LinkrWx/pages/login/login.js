...
Page({
  ...
  async login() {
    if (this.data.isPasswordLogin) {
      const res = await request('/api/auth/login', 'POST', this.data.passwordInfo);
      if (res && res.success) {
        await wx.setStorageSync('access_token', res.data && res.data.token);
        wx.switchTab({ url: `/pages/my/index` });
      }
    } else {
      const res = await request(`/api/auth/sms/send?phone=${this.data.phoneNumber}`, 'GET');
      if (res && res.success) {
        wx.navigateTo({
